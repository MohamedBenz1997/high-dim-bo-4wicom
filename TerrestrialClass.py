            ...
            if self.UAVs:
                if self.one_tier == False:
                    if self.indoor == False:
                        LSL_1, LSG_1, G_Antenna_1, p_LOS_1, pl_1, shadowing_LOS_1, shadowing_NLOS_1 = self.Large_Scale_Gain_drone.call(D[:, 0:19, 0:int(self.GUE_ratio * self.Nuser_drop) + 1],D_2d[:, 0:19, 0:int(self.GUE_ratio * self.Nuser_drop) + 1],Azi_phi_deg[:, 0:19, 0:int(self.GUE_ratio * self.Nuser_drop) + 1],Elv_thetha_deg[:, 0:19, 0:int(self.GUE_ratio * self.Nuser_drop) + 1], self.Zuser,self.BS_tilt, self.BS_HPBW_h)  # GUEs
                        LSL_2, LSG_2, G_Antenna_2, p_LOS_2, pl_2, shadowing_LOS_2, shadowing_NLOS_2 = self.Large_Scale_Gain_drone.call(D[:, 0:19, int(self.GUE_ratio * self.Nuser_drop) + 1:],D_2d[:, 0:19, int(self.GUE_ratio * self.Nuser_drop) + 1:],Azi_phi_deg[:, 0:19, int(self.GUE_ratio * self.Nuser_drop) + 1:],Elv_thetha_deg[:, 0:19, int(self.GUE_ratio * self.Nuser_drop) + 1:],tf.expand_dims(Xuser[:, int(self.GUE_ratio * self.Nuser_drop)+1:, 2], axis=1),self.BS_tilt, self.BS_HPBW_h)  # UAVs

                    if self.indoor == True:
                        LSL_1, LSG_1, G_Antenna_1, p_LOS_1, pl_1, shadowing_LOS_1, shadowing_NLOS_1 = self.LSGclass.call(D[:, 0:19, :],D_2d[:, 0:19, :],D_in[:, 0:19, :], D_2d_in[:, 0:19, :],D_2d_building[:, 0:19, :],Azi_phi_deg[:,0:19, :],Elv_thetha_deg[:,0:19, :],Azi_phi_deg_in[:, 0:19, :],Elv_thetha_deg_in[:, 0:19, :],Zuser_indoor[:, 0:19, :], self.BS_tilt, self.BS_HPBW_h)
                        LSL_2, LSG_2, G_Antenna_2, p_LOS_2, pl_2, shadowing_LOS_2, shadowing_NLOS_2 = self.Large_Scale_Gain_drone.call(D_UAV[:, 0:19, :],D_2d_UAV[:, 0:19, :],Azi_phi_deg_UAV[:, 0:19, :],Elv_thetha_deg_UAV[:, 0:19, :], self.Zuav, self.BS_tilt, self.BS_HPBW_h)
